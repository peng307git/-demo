   ...
   proto.clickFullScreen = function(){
    ...
    proto.slideDown = function(){
     ...
     proto.homeplay = function(){
        ...
        if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){ 
            _self.video.addEventListener("fullscreenchange", FShandler);  
            _self.video.addEventListener("webkitfullscreenchange", FShandler);  
            _self.video.addEventListener("mozfullscreenchange", FShandler);  
            _self.video.addEventListener("MSFullscreenChange", FShandler); 
        }
            function FShandler() { 
                if(status == 0 ){
                    _self.hiderVideo();
