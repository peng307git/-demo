...
var addressCountry=function(data){
	var _self = this;
	_self.ider = $(document).find(data.id);
	_self.needId = false;
	if (data.showId) {
...
proto.flagEvent = function(){
    ...
    for(var i in _self.flags_data)
    {
    	...
    	var class_str='';
    	htmlCountry +='<option value ="'+i+'">'+country+''+i+'</option>'
    }
    ...
    $.i18n.properties({
            ...
            callback : function() {//加载成功后设置显示内容
            	if(_self.needId == true ){
            		for(var i in _self.flags_data)
	            	{
	                  _self.ider.find('option[value='+i+']').text(' '+$.i18n.prop(''+i)+' +'+i);
	            	}
            	}else{
	            	for(var i in _self.flags_data)
	            	{
	                  _self.ider.find('option[value='+i+']').text(' '+$.i18n.prop(''+i));
	            	}
            	}
            }
    });
