$(function() {
	var stayCoreNum = sideListArr.length - 1;
            ...
            this.group.append('image')
                ...
                .on('click', function() {
                    if (self.data.status == 3 || self.data.status == 4 || (self.data.status == 2 && self.data.userid == userid)) {
                        ...
                        if(  percentage == 100 && self.data.is_mp_receive == 1 ){
                            $("#documentAlertMark").show();
                            alertBoxFunc(I18N.prop('alertnoticeError'), "sure",function(){
                                   $("#documentAlertMark").hide(); 
                            });
                        }else{
                ...
                success: function(obj) {
                    if (0 == obj.status) {
                        ...
                        var textArr = [{
                            ...
                            cx: 60,
                            cy: 45,
                            text: I18N.prop('nodeoccupy') + ':' + Math.floor(accMul(parseFloat(obj.data.use_proportion), 100)) + '%'
                        }];
                    ...
                    success: function(obj) {
                        if (0 == obj.status) {
                            ...
                            if (detailData.capacity != null) {
                                detailData.capacity = accMul(detailData.capacity, 100) + '%';
                            }
                            if (detailData.contribution_output != null) {
                                detailData.contribution_output = accMul(detailData.contribution_output, 100) + '%';
                            }
        ...
        showCreatePop: function(index) {
            ...
            */
            ...
            this.createPopGroup.selectAll("text")
                ...
                .on('click', function(d, i) {
                    if (0 == i) {
                        if ($('#userAllCapacity').val() >= 200) {
                            $('#createSubCoreModal').modal('show');
                        } else {
                            alertBoxNew({
                                alertType: 'warning',
                                alertContent: 'alertStr11',
                                btnType: 'sure',
                            })
                        }
                    } else {
    ...
    SCENE.prototype = {
        ...
        showMineStatus: function(data) {
            if (data.capacity != null) {
                var stayCapacity = data.capacity - stayCoreNum;
                var stayCore = parseInt((data.capacity - stayCoreNum)) > 0 ? parseInt((data.capacity - stayCoreNum)) : 0;
                data.capacity = accMul(data.capacity, 100) + '%';
            }
            ...
            if ($('.mine-side-bar > li:first').hasClass('active') && (stayCore > 0)) {
                $('#userAllCapacity').val(stayCapacity * 100);
                $('#moadlCapacity').html(data.capacity);
                $('#modalSeedCore').html(stayCoreNum);
                $('#moadlStayCore').html(stayCore - 1);
            }
        },
    ...
    $("#documentAlertMark").on('click',function(){
        $(this).hide();
        $('#alertWrap').remove();
    })
});